...
def makeFigureS9_All5HMMparams():
    ...
    CMIP = loadData('CMIP_SOWs')
    CMIPunscaled = loadData('CMIPunscaled_SOWs')
    ...
    CMIP['Ensemble'] = 'CMIP with VIC Multipliers'
    CMIPunscaled['Ensemble'] = 'CMIP with VIC Noise Model'
    Paleo['Ensemble'] = 'Paleo'
    Original_1000['Ensemble'] = 'Box Around Historical'
    Wider_1000['Ensemble'] = 'All-Encompassing'
    
    allSamples = pd.concat([Wider_1000,Original_1000,Paleo,CMIPunscaled,CMIP,Historical])
    col = allSamples.columns.tolist()
    
    sns.set_style("dark")
    
    colors=['#bebada','#fb8072','#b3de69','#ffffb3','#fdb462','#80b1d3']
    ...
    plt.savefig('FigureS9_All5HMMparams.pdf')
    plt.clf()
